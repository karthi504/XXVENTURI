create or replace procedure xxventuri.aws_new_salesdata_sync_tally is
...
begin
    ...
    merge into crm_accounts_v@awsdb3 x
    using (
        select
            unique_id,
            customer_id,
   ...
   /*** 2. Sync Data: crm_account_locations_v  ***/
    merge into crm_account_locations_v@awsdb3 x
    using (
        select
            customer_name,
            unique_id,
        ...
        select
            ...
            city_name
        from
            crm_account_locations_v@awsdb3
   ...
   /*** 3. Sync Data: crm_products_v  ***/
    merge into crm_products_v@awsdb3 x
    using (
        select
            product_id,
            product_code,
   ...
   /*** 4. Sync Data: crm_users_v  ***/
    merge into crm_users_v@awsdb3 x
    using (
        select
            unique_id,
            staff_code,
   ...
   /*** 5. Sync Data: crm_products_stock_cost_v  ***/
    begin
        delete from crm_products_stock_cost_v@awsdb3;

--      INSERT INTO crm_products_stock_cost_v@awsdb3
--         SELECT * FROM crm_products_stock_cost_v;
   ...
   /*** 7. Sync Data: xxeba_sales_order - Insert New Sales Orders to Sales Order table ***/
    merge into xxeba_sales_order@awsdb3 x
    using (
        select
            order_id,
            deal_id,
   ...
   /*** 8. Sync Data: crm_sales_orders_v ***/
    begin
        delete from crm_sales_orders_v@awsdb3;

        insert into crm_sales_orders_v@awsdb3
            select
                *
            from
                crm_sales_orders_v;
    ...
    commit;
end aws_new_salesdata_sync_tally;
/


-- sqlcl_snapshot {"hash":"b2cd220ab45957173b4d4d61bf29831002a5f556","type":"PROCEDURE","name":"AWS_NEW_SALESDATA_SYNC_TALLY","schemaName":"XXVENTURI","sxml":""}