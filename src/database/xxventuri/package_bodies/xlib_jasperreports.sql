create or replace package body xxventuri.xlib_jasperreports as
/*=========================================================================
  Purpose  : 
...
=========================================================================*/
   ...
   -- convert boolean to 'true' or 'false'
    ...
    procedure setup_configuration_defaults is
    ...
    begin
        ...
        if m_report_url is null then
            m_report_url := l_conf.conf_protocol
                            || '://'
                            || l_conf.conf_server
                            || ':'
                            || l_conf.conf_port
                            || '/'
                            || l_conf.conf_context_path
                            || '/report';

            xlog(
                p_module => m_module,
                p_msg    => 'Override report url from defaults: ' || m_report_url,
                p_type   => 'DEBUG'
            );
        ...
        if
            lower(m_report_url) like 'https%'
            and l_conf.conf_wallet_path is not null
        then
            utl_http.set_wallet(l_conf.conf_wallet_path, l_conf.conf_wallet_pwd);
            xlog(
                p_module => m_module,
                p_msg    => 'Override wallet location/pwd from defaults',
                p_type   => 'DEBUG'
            );
        end if;
        ...
        if ( l_timeout is null
             or l_timeout = 60 /*60 is the default*/ ) then
            utl_http.set_transfer_timeout(l_conf.conf_http_transfer_timeout);
            xlog(
                p_module => m_module,
                p_msg    => 'Override http transfer timeout ('
                         || l_timeout
                         || 's) from defaults: '
                         || l_conf.conf_http_transfer_timeout
                         || 's',
                p_type   => 'DEBUG'
            );
   ...
   ----------------------------------------------------------------------------
    ...
    function get_report_url return varchar2 is
    begin
        return m_report_url;
    end;
    ...
    */
    procedure use_images_no_tunnel (
        p_server_uri  in varchar2 default null,
        p_cookie_path varchar2 default null
    ) is
    begin
    ...
    function get_use_images_no_tunnel return boolean is
    begin
        return m_use_images_no_tunnel;
    end;

    function get_cookie_path_no_tunnel return varchar2 is
    begin
        return m_cookie_path_no_tunnel;
    end;

    function get_images_uri return varchar2 is
    begin
        return m_images_uri;
    end;
   ...
   ----------------------------------------------------------------------------
    procedure show_image (
        ...
        begin
            ...
            xlog(l_proc,
                 jsession_cookie.vals(1));
            xlog(l_proc,
                 jsession_path_cookie.vals(1));
    ...
    begin
      ...
      -- _image?image=img_0_0_15&uuid=b41eb881-7ca5-4919-bd8f-5afa8d10b398
        ...
        l_url := l_url
                 || '&JSESSIONID='
                 || owa_cookie.get(name => m_jri_cookie_name_c).vals(1);
   ...
   ----------------------------------------------------------------------------
    ...
    begin
      ...
      -------------------------------------------------------
        ...
        l_url := l_url
                 || '&_repTimeZone='
                 || apex_util.url_encode(p_rep_time_zone);

      -- per default use the tunnel through the database
        if m_images_uri is null
           or m_images_uri = c_images_uri_tunnel then
         ...
         -- use parameter, mostly for direct access to servlet
            l_url := l_url
                     || '&_imagesURI='
                     || apex_util.url_encode(m_images_uri);
        end if;
...
*/
      ...
      -- additional report parameter passed?
        if ( p_additional_params is not null ) then
...
--         l_url := l_url || '&' || p_additional_params;
   ...
   ----------------------------------------------------------------------------
    function get_default_configuration return xlib_jasperreports_conf%rowtype is
        l_conf xlib_jasperreports_conf%rowtype;
    begin
        select
            *
   ...
   ----------------------------------------------------------------------------
    procedure set_default_configuration (
        p_conf in out xlib_jasperreports_conf%rowtype
    ) is
    begin
        p_conf.conf_id := 'MAIN';
...
-- sqlcl_snapshot {"hash":"a668f5e4b9b530b6e4518c594a7621e6fffd34b2","type":"PACKAGE_BODY","name":"XLIB_JASPERREPORTS","schemaName":"XXVENTURI","sxml":""}