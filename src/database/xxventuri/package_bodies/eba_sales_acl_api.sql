create or replace package body xxventuri.eba_sales_acl_api as
    -------------------------------------------------------------------------
    ...
    begin
        -- Remove old errors
        delete from eba_sales_errors
        where
            err_time <= localtimestamp - 21;
        -- Log the error.
    ...
    begin
        ...
        if get_preference_value('ACCESS_CONTROL_ENABLED') = 'N' then
            ...
            if
                l_access_level_id = 1
                and get_preference_value('ACCESS_CONTROL_SCOPE') = 'PUBLIC_CONTRIBUTE'
            then
                return 2;
            end if;
        end if;
...
-- sqlcl_snapshot {"hash":"6d04ce2dada5f93f1222f7f9824f6ef74bedbf95","type":"PACKAGE_BODY","name":"EBA_SALES_ACL_API","schemaName":"XXVENTURI","sxml":""}